...
const Register: React.FC = () => {
  ...
  const registerData =
    !loading && !error
      ? data.register.map((unit: RegisterUnit) => {
          const start =
      ...
      {loading || error ? (
        ...
        <ul id="register">
          ...
          {data.register.length === 0 ? (
            <li className={styles.noItemsInfo}>
              {"Записи в журналі за " +
                [
                  registerDate.getDate(),
                  registerDate.getMonth() + 1,
