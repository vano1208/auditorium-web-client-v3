...
const GridElement: React.FC<PropTypes> = ({ classroom, onClose }) => {
  const { name: classroomName, occupied, schedule, disabled } = classroom;
  ...
  const disabledComment = disabled
    ? disabled.comment +
      " до " +
      new Date(disabled.until).getDate() +
      "." +
      (new Date(disabled.until).getMonth() + 1) +
      "." +
      new Date(disabled.until).getFullYear() +
      ", " +
      new Date(disabled.until).getHours() +
      ":" +
      formatMinutesToMM(new Date(disabled.until).getMinutes())
    : null;
  return (
    <div
      onClick={onClick}
    ...
    >
      {classroom.special && (
        <img className={styles.specialIcon} src={pianoIcon} alt="piano" />
      )}
      ...
      <div className={styles.message}>
        ...
        ) : occupiedBecauseSchedule ? (
          <p>Можливі заняття за розкладом</p>
        ) : (
          <p>Вільно</p>
        )}
      </div>
